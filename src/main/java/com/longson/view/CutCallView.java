 ...
 *  车间通过裁剪呼叫功能  呼叫裁片
 */
public class CutCallView extends VBox {
...
//                                             Orders order = (Orders) calltable.getSelectionModel().getSelectedItem();
//                System.out.println(orders);
            ...
            calltable.setOnMouseClicked(new EventHandler<MouseEvent>() {
                @Override
                public void handle(MouseEvent event) {
                    ...
                    if (button == MouseButton.SECONDARY) {
                        SuperMarketWip wip = (SuperMarketWip) calltable.getSelectionModel().getSelectedItem();

                        String params="username";
                        String value=username;
                        String append=String.format("?%s=%s",params,value);
                        String result= HttpUrlConnect.doGet("http://10.32.24.85:8010/web/Dept"+append);
//                        String url="http://localhost:8010/web/Dept"+append;
                         Dept dept = JSONObject.parseObject(result, Dept.class);
                         System.out.println(dept);
                         String deptname=dept.getDept();
                         String line=dept.getLine();

                         new AlertWindow(dept,wip);
            ...
            calltable.setTableMenuButtonVisible(true);
            tableBox.getChildren().addAll(calltable,calllist);
            tableBox.setMargin(calltable,new Insets(10,5,5,10));
            tableBox.setMargin(calllist, new Insets(30, 10, 10, 10));


//        sc.setPannable(true);

            this.getChildren().addAll(hBox,tableBox);
